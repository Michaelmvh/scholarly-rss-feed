...
| `view_author` | Browser-only tracked-author filter generated by the reader controls |
...
The browser reader is server-rendered HTML with no JavaScript or third-party assets. Its controls
can narrow the loaded publications by date, tracked author, and curated collection without making
another provider request. These view filters are preserved when opening an article and returning
