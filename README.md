...
controls retain an Apply button when JavaScript is unavailable. Date, author, and curated
collection controls narrow already-loaded publications without another provider request. The
preprint-category control is different: it opts into native bioRxiv and arXiv discovery and
therefore refreshes the feed. No native category is selected on the main reader by default.
Multiple selected authors and native categories use OR semantics within their respective groups.
These selections are preserved when opening an article, returning to the reader, and opening its
corresponding raw RSS.
