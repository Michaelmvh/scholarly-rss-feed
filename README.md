...
and links to the original article and any available open-access PDF. Unique paths prevent browser
reading modes from reusing a different page's extracted content. Tracked authors are highlighted
in both views using provider IDs when available and canonical names or configured aliases for
curated records; if several tracked authors contributed, each is highlighted.
