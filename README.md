...
| `view_source` | Browser-only curated-collection filter generated by the reader controls |
...
applies single-select filters when they change and author filters when the multiselect closes; the
same controls retain an Apply button when JavaScript is unavailable. Filters can narrow the loaded
publications by date, one or more tracked authors, and curated collection without making another
provider request. Multiple selected authors use OR semantics, while date and collection filters
combine with them using AND semantics. The collection control groups registered collections and
can exclude papers found only through a curated collection while retaining provider results that
also occur in a collection. These view filters are preserved when opening an article and returning
to the feed, but do not alter the raw RSS. Selecting a publication opens an internal preview page
